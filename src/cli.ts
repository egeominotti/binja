...
import { resolveContained } from './index'
...
function createTemplateLoader(baseDir: string, extensions: string[]): TemplateLoader {
  return {
    load(name: string): string {
      const basePath = resolveContained(baseDir, name)
      if (basePath === null) {
        throw new Error(`Template name escapes the source directory: ${name}`)
      }
      for (const ext of [...extensions, '']) {
        const fullPath = basePath + ext
        if (fs.existsSync(fullPath) && resolveContained(baseDir, fullPath) !== null) {
...
const blockedProperties = new Set(['__proto__', 'prototype', 'constructor', 'caller', 'callee', 'arguments']);
const resolveName = (ctx, name) =>
  !blockedProperties.has(name) && Object.hasOwn(ctx, name) ? ctx[name] : undefined;
const getProperty = (object, key) => {
  if (object == null || (typeof key === 'string' && blockedProperties.has(key))) return undefined;
...
const escape = (v) => {
  if (v == null) return '';
  if (v?.__safe__ || v?.__safe) return v;
  const escaped = Bun.escapeHTML(String(v));
  const safe = new String(escaped);
  safe.__safe__ = true;
  return safe;
};

const stringify = (v, autoescape) => {
  if (v == null) return '';
  if (v?.__safe__ || v?.__safe) return String(v?.value ?? v);
  if (typeof v === 'boolean') return v ? 'True' : 'False';
  if (!autoescape) return String(v);
...
async function compileFile(
  ...
  try {
    ...
    if (!check.canFlatten) {
      throw new Error(`${check.reason}. Use runtime rendering for dynamic template names.`)
    }
    const finalAst = flattenTemplate(ast, { loader })
      ...
      if (entry.isDirectory()) {
        ...
        if (options.extensions.includes(ext)) {
          ...
          try {
            ...
            if (check.canFlatten) {
              const flattened = flattenTemplate(ast, { loader })
              compileToString(flattened)
              canCompile++
...
async function lintTemplates(sourcePath: string, isDirectory: boolean, options: LintCliOptions) {
  ...
  if (options.ai) {
    try {
      ...
      error('  - GROQ_API_KEY + GROQ_MODEL (optional)')
