...
describe('Django Extra Filters', () => {
  describe('addslashes', () => {
    ...
    test('handles mixed escapes', async () => {
      const result = await render("{{ value|addslashes }}", { value: "it's \"quoted\" with \\backslash" })
  ...
  describe('json_script', () => {
    test('outputs JSON in script tag', async () => {
      ...
      const result = await env.renderString("{{ data|json_script:'my-data' }}", { data: { name: 'test' } })
    ...
    test('escapes HTML entities in JSON', async () => {
      const env = new Environment({ autoescape: false })
      const result = await env.renderString("{{ data|json_script:'data' }}", { data: { html: '<script>' } })
      expect(result).toContain('\\u003Cscript\\u003E')
  ...
  describe('truncatechars_html', () => {
    test('truncates preserving HTML tags', async () => {
      const env = new Environment({ autoescape: false })
      const result = await env.renderString("{{ value|truncatechars_html:8 }}", { value: '<p>Hello World</p>' })
      expect(result).toBe('<p>Hello Wo...</p>')
    })

    test('handles nested tags', async () => {
      const env = new Environment({ autoescape: false })
      const result = await env.renderString("{{ value|truncatechars_html:9 }}", { value: '<div><b>Bold</b> text</div>' })
    ...
    test('does not truncate short content', async () => {
      const env = new Environment({ autoescape: false })
      const result = await env.renderString("{{ value|truncatechars_html:100 }}", { value: '<p>Short</p>' })
  ...
  describe('truncatewords_html', () => {
    test('truncates by words preserving HTML', async () => {
      const env = new Environment({ autoescape: false })
      const result = await env.renderString("{{ value|truncatewords_html:3 }}", { value: '<p>One two three four five</p>' })
      expect(result).toBe('<p>One two three...</p>')
    })

    test('handles nested tags', async () => {
      const env = new Environment({ autoescape: false })
      const result = await env.renderString("{{ value|truncatewords_html:2 }}", { value: '<div><b>Bold</b> text here</div>' })
      expect(result).toBe('<div><b>Bold</b> text...</div>')
    })
  })
  ...
  describe('urlizetrunc', () => {
    test('converts URLs and truncates display text', async () => {
      const env = new Environment({ autoescape: false })
      const result = await env.renderString("{{ value|urlizetrunc:15 }}", { value: 'Visit https://www.example.com/very/long/path' })
...
describe('Django Extra Tags', () => {
  ...
  describe('{% cycle %}', () => {
    test('cycles through values', async () => {
      const result = await render("{% for i in items %}{% cycle 'odd' 'even' %}{% endfor %}", { items: [1, 2, 3, 4] })
      expect(result).toBe('oddevenoddeven')
    })

    test('cycles with as variable', async () => {
      const result = await render("{% for i in items %}{% cycle 'a' 'b' as letter %}{{ letter }}{% endfor %}", { items: [1, 2, 3] })
  ...
  describe('{% firstof %}', () => {
    test('outputs first truthy value', async () => {
      const result = await render("{% firstof var1 var2 var3 %}", { var1: '', var2: null, var3: 'found' })
  ...
  describe('{% ifchanged %}', () => {
    test('outputs on change', async () => {
      const result = await render("{% for item in items %}{% ifchanged %}{{ item }}{% endifchanged %}{% endfor %}", { items: ['a', 'a', 'b', 'b', 'c'] })
      expect(result).toBe('abc')
    })

    test('outputs else on no change', async () => {
      const result = await render("{% for item in items %}{% ifchanged %}NEW{% else %}same{% endifchanged %}{% endfor %}", { items: ['a', 'a', 'b'] })
    ...
    test('outputs with value comparison', async () => {
      const result = await render("{% for item in items %}{% ifchanged item %}changed{% else %}same{% endifchanged %}{% endfor %}", { items: ['a', 'a', 'b', 'b'] })
...
`.trim(), {
  ...
  describe('{% widthratio %}', () => {
    test('calculates ratio', async () => {
      const result = await render("{% widthratio value max_value 100 %}", { value: 50, max_value: 200 })
      expect(result).toBe('25')
    })

    test('stores in variable with as', async () => {
      const result = await render("{% widthratio value max_value 100 as ratio %}{{ ratio }}", { value: 75, max_value: 100 })
      expect(result).toBe('75')
    })

    test('handles zero max value', async () => {
      const result = await render("{% widthratio value max_value 100 %}", { value: 50, max_value: 0 })
  ...
  describe('{% ifequal %} / {% ifnotequal %}', () => {
    test('ifequal matches equal values', async () => {
      const result = await render("{% ifequal a b %}equal{% else %}not{% endifequal %}", { a: 'test', b: 'test' })
      expect(result).toBe('equal')
    })

    test('ifequal does not match different values', async () => {
      const result = await render("{% ifequal a b %}equal{% else %}not{% endifequal %}", { a: 'test', b: 'other' })
      expect(result).toBe('not')
    })

    test('ifnotequal matches different values', async () => {
      const result = await render("{% ifnotequal a b %}different{% else %}same{% endifnotequal %}", { a: 'x', b: 'y' })
      expect(result).toBe('different')
    })

    test('ifnotequal does not match equal values', async () => {
      const result = await render("{% ifnotequal a b %}different{% else %}same{% endifnotequal %}", { a: 5, b: 5 })
