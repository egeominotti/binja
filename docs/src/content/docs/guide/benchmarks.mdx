...
The published comparison snapshot below was recorded on a Mac Studio M1 Max with Bun 1.3.5.
Use the repository benchmark command to measure the current checkout and Bun version locally.
...
The command reports warmed-up median results for cached runtime rendering, AOT rendering,
Liquid loop modifiers, and algorithm-sensitive filters. It consumes every output to prevent
the JavaScript engine from eliminating work. Results vary by hardware and Bun version, so use
the same machine and runtime when comparing commits.
